...
> These retained numerical request/result cells are not execution evidence.
...
structure and returns its two public pins.
...
`floating_plus_pin` and `floating_minus_pin` are the child boundary. The root
cell next links them and creates the coordinate aliases used by the shared View.
