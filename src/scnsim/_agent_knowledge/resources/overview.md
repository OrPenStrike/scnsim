...
Built wheels and source distributions also carry version-bound public
documentation and example sources under `scnsim/_agent_knowledge/`. Tools can
read those UTF-8 resources through installed distribution metadata without
importing SCNSim or launching Julia. See the
[installed-package usage guide](docs/agent-usage.md) for resource discovery,
support boundaries, and a complete Direct example.
...
- [Tutorial](docs/index.qmd) — short TARGET lessons grouped into
  ...
  needs.
- [Installed-package usage](docs/agent-usage.md) — agent-readable resources,
  version provenance, and a bounded executable Direct example.
