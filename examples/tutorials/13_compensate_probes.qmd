...
![Default authoring projection of the complete raw-loaded capstone.](figures/13-ptc-authoring.svg){#fig-13-ptc-authoring .lightbox fig-alt="Feedline, readout, and floating subsystems with all four original Ports before probe compensation."}

[Open the SVG](figures/13-ptc-authoring.svg).

:::

Fixed Default is not a universal embedding algorithm: this complete Plan can
encounter an unsupported fixed track near a label. The cell reports an actual
layout failure, if encountered, and continues to explicit controls below;
other error stages propagate. It does not substitute a drawing or an audit.
When rendering succeeds, `automatic` shows the raw authored Plan and its audit
